...
export default {
  ...
  methods: {
    ...
    appendCliente(){
      if (this.nameForm === undefined ||this.subnameForm === undefined ||this.dniForm === undefined ||this.emailForm === undefined ){
        this.mostrarAlerta('Debe Completar los Campos','warning')
        return false;
      }

      if(this.nameForm.trim() === '' ||this.subnameForm.trim() === '' ||this.dniForm.trim() === '' ||this.emailForm.trim() === '' ){
        this.mostrarAlerta('Debe Completar los Campos','warning')
        return false;
      }

      this.clientes.push({id:this.clientes+1, nombre: this.nameForm, apellido: this.subnameForm, dni: this.dniForm, email: this.emailForm});
    ...
    <table class="table table-striped ">
      ...
      <tr v-for='(cliente) in clientes' :key="cliente.id">
        ...
        <td class="d-flex gap-4 justify-content-center">
          <button class="btn btn-danger" @click="eliminarCliente(cliente.id)"><IconTrash></IconTrash></button>
          <button class="btn btn-warning" @click="modificarCliente(cliente.id)"><IconPencil></IconPencil></button>
        </td>
      </tr>
      </tbody>
    </table>
